...
export default function EditTutorialGuru() {
    const { tutorials } = usePage().props;

    const { data, setData, post, processing, errors } = useForm({
        id: tutorials.id,
        judul: tutorials.judul,
        sumber: tutorials.sumber,
        cover: tutorials.cover,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        triggerAlert();
        post(route("update.tutorial"));
    };

    const triggerAlert = () => {
        Swal.fire({
            icon: "success",
            title: "Tutorial Berhasil Diedit",
    ...
    return (
        <MainGuruLayout>
            <Head title="Tutorial" />
            <h1 className="font-semibold text-2xl mb-10">Edit Tutorial</h1>
